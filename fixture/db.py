...
class DbFixture:
    ...
    def get_contact_list_from_group(self):  # загружаем список контактов из базы данных
        list=[]  # инициализируем список
        cursor = self.connection.cursor()
        try:
            cursor.execute("select id, firstname, middlename, lastname, nickname, company, title, address, home, mobile, work, email, email2, email3, homepage, address2, phone2 from address_in_groups where deprecated = '0000-00-00 00:00:00'")
            for row in cursor:
                (id, firstname, middlename, lastname, nickname, company, title, address,
                home, mobile, work, email, email2, email3, homepage, address2, phone2 ) = row
                list.append(Contact(id=str(id), firstname=firstname, middlename=middlename, lastname=lastname, nickname=lastname,
